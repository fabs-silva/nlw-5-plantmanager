...
export async function savePlant(plant: PlantProps): Promise<void> {
  try {
    ...
    const data = await AsyncStorage.getItem('@plantmanaver:plants');
    ...
    await AsyncStorage.setItem('@plantmanaver:plants',
      JSON.stringify({
        ...newPlant,
        ...oldPlants
      })
...
export async function loadPlants(): Promise<PlantProps[]> {
  try {
    const data = await AsyncStorage.getItem('@plantmanaver:plants');
    const allPlants = data ? (JSON.parse(data) as StoragePlantProps) : {};
